...
byte btnArr[] = {dPinLapMode,A3,A4,A5,9,10,11};
#define btnArrCt 7
...
const byte tmClock = 7;
const byte tmData = 6; 
const byte tmStrobe0 = 5; 
#define TM_LED_OFF 0
...
void OnPjonRemoveAll() {
  ...
  pjonNetwork.remove_all(device_id);
}
...
*/
  ...
  pinMode(dPinLapMode,INPUT_PULLUP);
  pinMode(dPinLapUp,INPUT_PULLUP);
  pinMode(dPinLapDown,INPUT_PULLUP);
...
void loop() 
{
  ...
  if (pjonBufCtLast != pjonBufCt) {
    ...
    cmdMessenger.sendCmd(kPjonBufferCt,PJON_2*1000 + pjonNetwork.get_packet_count(PJON_2));
    cmdMessenger.sendCmd(kPjonBufferCt,PJON_3*1000 +pjonNetwork.get_packet_count(PJON_3));
    cmdMessenger.sendCmd(kPjonBufferCt,PJON_4*1000 +pjonNetwork.get_packet_count(PJON_4));
  }
